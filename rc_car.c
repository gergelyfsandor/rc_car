...
void LSensor_Task(void) {
    ...
    if ((InputBuffers.SIA_LightSensor < (LIGHT_THRESHOLD - LIGHT_THRESHOLD_HIST))
            && (isLowBeamOn == 0)) {
        isLowBeamOn = 1;
...
void Flasher_Task(void) {
    ...
    static enum sm {
        ...
        state_fleft_on,
        state_fleft_off,
        state_flash_switch_off
    } StateMachine = state_inactive;

    //Check if there is any noew command to execute
    if (xQueueCRReceive(Flash_CommandHandle, &flashCommand, 10000)) {
        switch (flashCommand) {
        case Flash_Stop:
...
void Controller_Task(void) {
    ...
    //update PWM values
    if (updateMotor) {
        SET_PWM1(OutputBuffers.SODPWM_EnableMotor1); SET_PWM2(OutputBuffers.SODPWM_EnableMotor2);
    }
